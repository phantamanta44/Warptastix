...
public class SetWarpCommand extends WTXCommand {
    ...
    protected void execute(CommandSender sender, String[] args) throws WTXCommandException {
        ...
        if (opts.has("p") && !sender.hasPermission("warptastix.setwarp.private"))
            throw new WTXCommandException(WTXLang.prefix("noperms"));
        if (opts.has("s")) {
            ...
            if (warp != null) {
                if (warp.getOwner() == null) {
                    throw new WTXCommandException(WTXLang.prefix("command.setwarp.server.already", warp.getName()));
                } else {
                    warp.setOwner(null);
                    warp.setPriv(false);
                    Warptastix.wdb().save();
                    sender.sendMessage(WTXLang.prefix("command.setwarp.server.assimilate", warp.getName()));
                }
            } else {
                Warptastix.wdb().add(warp = new Warp(args[0], (Player)sender, false));
                Warptastix.wdb().save();
                sender.sendMessage(WTXLang.prefix("command.setwarp.server.set", warp.getName()));
            }
        } else {
            ...
            if (warp != null) {
                if (opts.has("p")) {
                    ...
                    if (owner.getUniqueId().equals(warp.getOwner())) {
                        ...
                        Warptastix.wdb().save();
                        sender.sendMessage(WTXLang.prefix("command.setwarp.privtoggle",
                                WTXLang.localize(warp.isPriv() ? "on" : "off"), warp.getName()));
                    } else {
                        sender.sendMessage(WTXLang.prefix("command.setwarp.notowner",
                                Bukkit.getServer().getOfflinePlayer(warp.getOwner()).getName()));
                    }
                } else {
                    throw new WTXCommandException(WTXLang.prefix("command.setwarp.already", warp.getName()));
                }
            } else {
                flushConditions();
                ...
                if (owner.equals(sender))
                    sender.sendMessage(WTXLang.prefix("command.setwarp.set", warp.getName()));
                else
                    sender.sendMessage(WTXLang.prefix("command.setwarp.set.other", warp.getName(), owner.getName()));
            }
        }
    }
